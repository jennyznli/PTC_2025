...
# saveRDS(deg_all, here("data", "20250302_thyroid_deg_all.rds"))

####
deg_all <- readRDS(here("data", "20250302_thyroid_deg_all.rds")) #22516     6
...
# ========================
res <- readRDS(here("diff_meth", "20250302_thyroid88_res_invasiveness_condensed_BH.rds"))
# res <- res %>% filter(
...
saveRDS(hyper_enrichment, here("diff_meth", "20250302_hyper_invasiveness_tfbs.rds"))
...
saveRDS(hypo_enrichment, here("diff_meth", "20250302_hypo_invasiveness_tfbs.rds"))

## =============================================
hyper_enrichment <- readRDS(here("diff_meth", "20250302_hyper_invasiveness_tfbs.rds"))
...
hypo_enrichment <- readRDS(here("diff_meth", "20250302_hypo_invasiveness_tfbs.rds"))
...
# ========================
# FIG 2E. TFBS VOLCANO PLOT
# ========================
deg_all <- readRDS(here("data", "20250302_thyroid_deg_all.rds")) # 22516 x 6
deg_all <- deg_all %>%
    mutate(
...
pdf(here("figures", "20250302_volcano_tfbs_rnaseq.pdf"), width=6.5, height=6, onefile=FALSE)
...
##  _____________
deg_stringent <- readRDS(here("data", "20250204_thyroid_deg_stringent.rds"))

deg_hyper_expressed <- deg_stringent %>%
    filter(rownames(.) %in% tf_hyper)  #0
deg_hypo_expressed <- deg_stringent %>%
    filter(rownames(.) %in% tf_hypo) #16

deg_tf_expr <- mtx[unique(c(rownames(deg_hypo_expressed), rownames(deg_hyper_expressed))), ] ## 16 84

# saveRDS(deg_tf_expr, here("data", "20250302_deg_tf_expr.rds"))

## START HERE _____________
deg_tf_expr <- readRDS(here("data", "20250302_deg_tf_expr.rds")) #25 84
ss <- read_excel(here("ss", "20231102_thyroid_master.xlsx")) %>%
    filter(Include_In_Analysis == "1") %>%
...
pdf(here("figures", "20250302_thyroid_heatmap_rnaseq.pdf"), width=8, height=6, onefile=FALSE)
...
pdf(here("figures", "20250418_gsea_enrichment_dot_hallmark.pdf"), width=6, height=4, onefile=FALSE)
